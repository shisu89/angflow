...
export const AGENT_TOOL_SCHEMAS: AgentToolSchema[] = [
  ...
  {
    "name": "get_state",
    "description": "Return a snapshot of a flow: { nodes, edges, viewport, collapsedHiddenIds }. collapsedHiddenIds is the board-wide set of nodes hidden because an ancestor group is collapsed (it stays board-wide even when groupId/bounds scopes nodes/edges). Optionally scope to a group subtree (groupId) OR a bounds rect (bounds) — not both. For a compact overview of a large board, prefer get_summary.",
    "inputSchema": {
      "type": "object",
      "properties": {
        "flowId": {
  ...
  {
    "name": "get_summary",
    "description": "Compact digest of a flow for large boards (avoids dumping every node). Returns { counts: { nodes, edges, groups }, groups: [{ id, label, collapsed, memberCount }], titles: [{ id, type, label }], viewport, bounds, collapsedHiddenIds }. titles carry id/type/label only (no data/style); use get_state (optionally scoped) for full node data.",
    "inputSchema": {
      "type": "object",
      "properties": {
        "flowId": {
  ...
  {
    "name": "add_node",
    "description": "Append a node to the flow. The node must include position { x, y } and data. Optional: type, width, height, draggable, selectable, hidden, etc. If id is omitted, the bridge mints and returns one.",
    "inputSchema": {
      "type": "object",
      "properties": {
        "flowId": {
  ...
  {
    "name": "group_nodes",
    "description": "Create a group (container) node wrapping the given nodes and reparent them into it (visually pinned). Mints and returns a groupId when none is supplied. Returns { groupId }.",
    "inputSchema": {
      "type": "object",
      "properties": {
        "flowId": {
        ...
        "nodeIds": {
          ...
          "description": "Members to group (non-empty)."
        },
        "label": {
          "type": "string",
          "description": "Group title (→ data.label)."
        },
        "collapsed": {
          "type": "boolean"
        },
        "groupId": {
          "type": "string",
          "description": "Optional id for the new group; minted when omitted."
        },
        "padding": {
          "type": "number",
          "description": "Box inset around members. Default 20."
        },
        "headerHeight": {
          "type": "number",
          "description": "Top inset for the title bar. Default 40."
        }
      },
      "required": [
        "nodeIds"
  ...
  {
    "name": "set_node_group",
    "description": "Reparent a node into a group (or detach to top-level with groupId null), keeping it visually fixed. Rejects cycles (-32602). Returns { nodeId, groupId }.",
    "inputSchema": {
      "type": "object",
      "properties": {
        "flowId": {
        ...
        "groupId": {
          ...
          "description": "Target group id, or null to detach."
        }
      },
      "required": [
        "nodeId",
  ...
  {
    "name": "dissolve_group",
    "description": "Remove a group node; its direct children survive (reparented to the group's parent, pinned in place). Returns { dissolvedGroupId, memberIds }.",
    "inputSchema": {
      "type": "object",
      "properties": {
        "flowId": {
  ...
  },
  {
    ...
    "description": "Poll the op-log: return mutating tool calls applied since a cursor. Returns { ops: [{ cursor, flowId, method, params, source? }], cursor, truncated }. Pass the returned cursor on the next call; truncated:true means entries were dropped (re-sync via get_state). Records only bridge-initiated mutations (not UI edits or undo/redo).",
    "inputSchema": {
      "type": "object",
      "properties": {
        "flowId": {
        ...
        "since": {
          "type": "number",
          "description": "Return ops with cursor greater than this. Omit/0 for all retained."
        }
      },
      "additionalProperties": false
    }
